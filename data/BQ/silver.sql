...
CREATE TABLE IF NOT EXISTS `vvns-bigquery.silver_dataset.departments` (
    Dept_Id STRING,
    SRC_Dept_Id STRING,
    Name STRING,
    datasource STRING,
...
TRUNCATE TABLE `vvns-bigquery.silver_dataset.departments`;

-- 3. full load by Inserting merged Data 
INSERT INTO `vvns-bigquery.silver_dataset.departments`
SELECT DISTINCT 
...
CREATE TABLE IF NOT EXISTS `vvns-bigquery.silver_dataset.providers` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
-- 2. Truncate Silver Table Before Inserting 
TRUNCATE TABLE `vvns-bigquery.silver_dataset.providers`;

-- 3. full load by Inserting merged Data 
INSERT INTO `vvns-bigquery.silver_dataset.providers`
SELECT DISTINCT 
    ProviderID,
    FirstName,
    LastName,
...
CREATE TABLE IF NOT EXISTS `vvns-bigquery.silver_dataset.patients` (
    Patient_Key STRING,
...
CREATE OR REPLACE TABLE `vvns-bigquery.silver_dataset.quality_checks` AS
SELECT DISTINCT 
...
MERGE INTO `vvns-bigquery.silver_dataset.patients` AS target
USING `vvns-bigquery.silver_dataset.quality_checks` AS source
...
DROP TABLE IF EXISTS `vvns-bigquery.silver_dataset.quality_checks`;

-------------------------------------------------------------------------------------------------------

-- 1. Create transactions Table in BigQuery
CREATE TABLE IF NOT EXISTS `vvns-bigquery.silver_dataset.transactions` (
...
-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `vvns-bigquery.silver_dataset.transactions` AS target
USING `vvns-bigquery.silver_dataset.quality_checks` AS source
...
CREATE TABLE IF NOT EXISTS `vvns-bigquery.silver_dataset.encounters` (
...
CREATE OR REPLACE TABLE `vvns-bigquery.silver_dataset.quality_checks_encounters` AS
SELECT DISTINCT 
...
-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `vvns-bigquery.silver_dataset.encounters` AS target
USING `vvns-bigquery.silver_dataset.quality_checks_encounters` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `vvns-bigquery.silver_dataset.quality_checks_encounters`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the Claims Table in BigQuery
CREATE TABLE IF NOT EXISTS `vvns-bigquery.silver_dataset.claims` (
    Claim_Key STRING,
    SRC_ClaimID STRING,
...
CREATE OR REPLACE TABLE `vvns-bigquery.silver_dataset.quality_checks_claims` AS
SELECT 
...
FROM (
    SELECT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `vvns-bigquery.bronze_dataset.claims`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `vvns-bigquery.silver_dataset.claims` AS target
USING `vvns-bigquery.silver_dataset.quality_checks_claims` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `vvns-bigquery.silver_dataset.quality_checks_claims`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the CP Codes Silver Table in BigQuery
CREATE TABLE IF NOT EXISTS `vvns-bigquery.silver_dataset.cpt_codes` (
    CP_Code_Key STRING,
...
CREATE OR REPLACE TABLE `vvns-bigquery.silver_dataset.quality_checks_cpt_codes` AS
SELECT 
...
FROM (
    SELECT 
        ...
        code_status,
        'hosa' AS datasource
    FROM `vvns-bigquery.bronze_dataset.cpt_codes`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `vvns-bigquery.silver_dataset.cpt_codes` AS target
USING `vvns-bigquery.silver_dataset.quality_checks_cpt_codes` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `vvns-bigquery.silver_dataset.quality_checks_cpt_codes`;
