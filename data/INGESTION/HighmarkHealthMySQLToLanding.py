...
spark = SparkSession.builder.appName("HighmarkHealthMySQLToLanding").getOrCreate()

# Google Cloud Storage (GCS) Confoguration
GCS_BUCKET = "healthcare-bucket-18082025"
HOSPITAL_NAME = "highmarkhealth"
...
# MySQL Configuration
MYSQL_CONFIG = {
    "url": "jdbc:mysql://34.63.79.254:3306/highmarkhealth_db?useSSL=false&allowPublicKeyRetrieval=true",
...
def get_latest_watermark(table_name):
    query = f"""
        ...
        FROM `{BQ_AUDIT_TABLE}`
        WHERE tablename = '{table_name}' and data_source = "highmarkhealth_db"
    """
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        ...
        audit_df = spark.createDataFrame([
            ("highmarkhealth_db", table, load_type, df.count(), datetime.datetime.now(), "SUCCESS")], 
...
for row in config_df.collect():
    if row["is_active"] == '1' and row["datasource"] == "highmarkhealth_db": 
