...
spark = SparkSession.builder.appName("ClevelandClinicMySQLToLanding").getOrCreate()

# Google Cloud Storage (GCS) Confoguration
GCS_BUCKET = "healthcare-bucket-18082025"
HOSPITAL_NAME = "clevelandclinic"
LANDING_PATH = f"gs://{GCS_BUCKET}/landing/{HOSPITAL_NAME}/"
ARCHIVE_PATH = f"gs://{GCS_BUCKET}/landing/{HOSPITAL_NAME}/archive/"
CONFIG_FILE_PATH = f"gs://{GCS_BUCKET}/configs/load_config.csv"

# BigQuery Configuration
BQ_PROJECT = "vvns-bigquery"
...
# MySQL Configuration
MYSQL_CONFIG = {
    "url": "jdbc:mysql://35.192.32.30:3306/clevelandclinic_db?useSSL=false&allowPublicKeyRetrieval=true",
...
def get_latest_watermark(table_name):
    query = f"""
        ...
        FROM `{BQ_AUDIT_TABLE}`
        WHERE tablename = '{table_name}' and data_source = "clevelandclinic_db"
    """
...
def extract_and_save_to_landing(table, load_type, watermark_col):
    try:
        ...
        audit_df = spark.createDataFrame([
            ("clevelandclinic_db", table, load_type, df.count(), datetime.datetime.now(), "SUCCESS")], 
...
for row in config_df.collect():
    if row["is_active"] == '1' and row["datasource"] == "clevelandclinic_db": 
